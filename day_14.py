...
reactions = {} # k = produced material, v = (prod_quantity, (needed_material, needed_quantity))

for line in input.split("\n"):
    ingredients, target = line.split("=>")

    targetQty, targetElem = target.split()

    ingrList = []

    for ingr in ingredients.split(","):
        iQty, iElem = ingr.split()

        ingrList.append((iElem, int(iQty)))

    reactions[targetElem] = (int(targetQty), ingrList)


have = collections.defaultdict(lambda : 0)
have["FUEL"] = -1 # negative means I need something

while True:
    # find something to look for
    for elem in have:
        if have[elem] < 0 and elem != "ORE":
            lookingFor = elem
            break
