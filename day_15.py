...
directions = {(1, 0): 2, (-1, 0): 1, (0, 1): 4, (0, -1): 3}
opposite = {1:2, 2:1, 3:4, 4:3}

toVisit = queue.LifoQueue() # DFS
toVisit.put((4, (0, -1)))
...
while not toVisit.empty():
    ...
    commandToGetThere, position = toVisit.get()

    #print(commandToGetThere, position)

    input.put(commandToGetThere)
    ...
    if resp == 0:
        board[position] = 'X'
        toVisit.get() # no need to come back
        continue
    elif resp == 1:
        board[position] = '.'
    elif resp == 2:
    ...
    for direction in directions:
        ...
        if newPos not in board:
            print("Visiting", newPos)

            toVisit.put((opposite[directions[direction]], position))
            toVisit.put((directions[direction], newPos))
            board[newPos] = "N"
...
while not path.empty():
    newPos, newDistance = path.get()
    if newPos not in board or newPos in visited or board[newPos] == 'X':
        continue
